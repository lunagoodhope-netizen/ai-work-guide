...
const tutorVisuals={1:'assets/tutor-step1-goal.png',2:'assets/tutor-step2-routine.png',3:'assets/tutor-step3-reminder.png',4:'assets/tutor-step4-sheet.png',5:'assets/tutor-step5-dashboard.png'};
function escapeTutorText(value){return String(value||'').replace(/[&<>\"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#039;'}[char]))}
function tutorPromptLines(value){const lines=String(value||'').split(/\n+/).map(line=>line.trim()).filter(Boolean);return lines.length?lines:[String(value||'')]}
function tutorPopup(step){const p='step'+step+'_';const image=tutorVisuals[step]?'<img src="'+tutorVisuals[step]+'" alt="STEP '+step+' 예시 화면">':'';const prompt=tutorPromptLines(tutorCopy[p+'prompt']).map(line=>'<p>'+escapeTutorText(line)+'</p>').join('');let content='<p class="tutor-popup-kicker">STEP '+step+'</p><h2 class="tutor-popup-title">'+escapeTutorText(tutorCopy[p+'title'])+'</h2><p class="tutor-popup-desc">'+escapeTutorText(tutorCopy[p+'desc'])+'</p><div class="tutor-popup-grid'+(image?'':' single')+'"><div><section class="tutor-prompt" aria-label="프롬프트 예시"><span class="tutor-prompt-label">복사해서 시작하기</span><div class="tutor-prompt-lines">'+prompt+'</div><button class="tutor-copy-button" type="button" data-tutor-copy>프롬프트 복사</button></section></div>'+image+'</div>';
...
document.querySelector('#tutor-modal-content')?.addEventListener('click',async e=>{const button=e.target.closest('[data-tutor-copy]');if(!button)return;const step=document.querySelector('.tutor-popup-kicker')?.textContent.replace('STEP ','');const prompt=tutorCopy['step'+step+'_prompt'];try{await navigator.clipboard.writeText(prompt);button.textContent='복사 완료'}catch(e){button.textContent='복사 실패'}setTimeout(()=>button.textContent='프롬프트 복사',1500)});
